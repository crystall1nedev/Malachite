...
public class MalachiteViewUtils : NSObject {
    ...
    public func createAndAddButtonToView(symbolName: String, delegate: UIViewController, view: UIView, utilities: MalachiteClassesObject, action: Selector, dimensions: [ CGFloat ], constraints: MalachiteViewUtils.buttonBuilder.constraints) -> UIButton {
        ...
        button.pointerStyleProvider = { button, proposedEffect, proposedShape -> UIPointerStyle? in
            ...
            let shapePath = UIBezierPath(roundedRect: button.bounds, cornerRadius: dimensions[0])
            ...
            return UIPointerStyle(effect: .lift(preview), shape: .roundedRect(rect, radius: dimensions[0]))
        }
        
        button.addTarget(delegate, action: action, for: .touchUpInside)
        button.addTarget(utilities.haptics, action: #selector(utilities.haptics.buttonMediumHaptics(_:)), for: .touchUpInside)
        ...
        if let LXA = constraints.LXA, let LXC = constraints.LXC, let LXP = constraints.LXP {
            if LXP { constraintsToAdd.append(button.leadingAnchor.constraint(equalTo: LXA, constant: LXC))
            } else { constraintsToAdd.append(button.trailingAnchor.constraint(equalTo: LXA, constant: LXC)) }
        }
        if let LYA = constraints.LYA, let LYC = constraints.LYC, let LYP = constraints.LYP {
            if LYP { constraintsToAdd.append(button.bottomAnchor.constraint(equalTo: LYA, constant: LYC))
            } else { constraintsToAdd.append(button.topAnchor.constraint(equalTo: LYA, constant: LYC)) }
        }
        
        if let CXA = constraints.CXA, let CXC = constraints.CXC { constraintsToAdd.append(button.centerXAnchor.constraint(equalTo: CXA, constant: CXC)) }
        if let CYA = constraints.CYA, let CYC = constraints.CYC { constraintsToAdd.append(button.centerYAnchor.constraint(equalTo: CYA, constant: CYC)) }
    ...
    /// Function that returns a slider for the user interface.
    public func createAndAddSliderToView(delegate: UIViewController, view: UIView, utilities: MalachiteClassesObject, action: Selector, dimensions: [ CGFloat ]) -> UISlider {
        let slider = UISlider()
        slider.translatesAutoresizingMaskIntoConstraints = false
        slider.addTarget(delegate, action: action, for: .valueChanged)
        slider.addTarget(utilities.haptics, action: #selector(utilities.haptics.buttonMediumHaptics(_:)), for: .touchUpInside)
        view.addSubview(slider)
        
        
        NSLayoutConstraint.activate([
            slider.widthAnchor.constraint(equalToConstant: dimensions[0]),
            slider.heightAnchor.constraint(equalToConstant: dimensions[1]),
            slider.centerYAnchor.constraint(equalTo: view.centerYAnchor),
            slider.centerXAnchor.constraint(equalTo: view.trailingAnchor, constant: -105),
        ])
    ...
    public struct buttonBuilder {
        ...
        public struct constraints {
            let LXA: NSLayoutXAxisAnchor?
            let LXC: CGFloat?
            let LXP: Bool?
            let LYA: NSLayoutYAxisAnchor?
            let LYC: CGFloat?
            let LYP: Bool?
            let CXA: NSLayoutXAxisAnchor?
            let CXC: CGFloat?
            let CYA: NSLayoutYAxisAnchor?
            let CYC: CGFloat?
        }
    }
    
    public struct tooltipBuilder {
        let text: String
        let anchor: CGFloat
